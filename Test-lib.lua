--[[
	================================================================
	 ...
	 ================================================================
	 CATATAN PERUBAHAN v5.0.0 — UPDATE BESAR
	 Seluruh API & kompatibilitas versi sebelumnya DIPERTAHANKAN.
	 Tidak ada fitur lama yang dihapus.
	 ================================================================
	 1. WEBHOOK — dirombak jadi modul `Webhook` terpisah & modular:
	    - Validasi format URL webhook Discord sebelum kirim apapun.
	    - Soft-timeout (8 detik) tiap percobaan kirim, gak nge-hang.
	    - `EWEHUB:SendWebhook(...)` sekarang return (ok, alasanGagal)
	      biar caller bisa kasih pesan error yang lebih jelas.
	    - Tombol "🧪 Test Webhook" di tab bawaan sekarang validasi URL
	      dulu & kasih feedback status yang lebih jelas (loading/gagal).
	 2. DISCORD EMBED — output webhook sekarang embed modern:
	    Author/Header (AuthorName+AuthorIcon), Field, Accent Color,
	    Footer (+icon opsional), Thumbnail/Icon, dan Timestamp otomatis
	    (bisa dimatikan lewat Timestamp = false).
	 3. BARU: `Tab:CreateNote({ Title=, Content= })` — komponen info
	    statis read-only. Gak ada Callback, gak dukung format apapun
	    (Lua/Markdown/HTML/RichText), desain ngikut tema library.
	 4. NOTIFY DIROMBAK TOTAL, TETAP 100% BACKWARD-COMPATIBLE:
	    - `Icon` (emoji/teks pendek di samping judul)
	    - `Color` (accent border+judul+progress bar per-notifikasi)
	    - `Position` (TopRight/TopLeft/BottomRight/BottomLeft/
	      TopCenter/BottomCenter — masing-masing container sendiri)
	    - `Animation` ("Fade" default/"Slide"/"Pop")
	    - `ShowProgress` (progress bar countdown otomatis)
	    - Queue system: banyak toast di posisi sama otomatis tertumpuk
	      rapi (gak perlu antrian manual — tumpukan visual = antrian).
	    Panggilan lama `EWEHUB:Notify({Title=,Content=,Duration=})`
	    tetap tampil identik seperti sebelumnya (default Position=
	    BottomRight, Animation=Fade, ShowProgress=false).

	 CATATAN PERUBAHAN v4.8.0:
	 1. BARU: tab pinned "🪝 Webhook" — selalu ada di setiap window,
	    TAPI fiturnya opsional (gak wajib diisi). Isinya: input URL
	    webhook Discord, toggle aktif/nonaktif, dan tombol Test Webhook.
	    URL & status aktifnya "terdaftar" lewat Flag biasa (WebhookUrl/
	    WebhookEnabled), jadi otomatis ikut sistem Save/Load Config yang
	    sudah ada — gak perlu API registrasi baru.
	 2. BARU: EWEHUB:SendWebhook(payload) — kirim embed Discord dari
	    script kapan aja (payload: Title, Description, Color, Fields,
	    Footer, Thumbnail, Username, AvatarUrl). Otomatis no-op kalau
	    Webhook belum diaktifkan/URL belum diisi. Coba beberapa fungsi
	    request umum di executor dulu (request/http_request/syn.request)
	    sebelum fallback ke HttpService:PostAsync biasa.
...
--============================================================
-- WEBHOOK MODULE (v5.0) — dipakai Tab "🪝 Webhook" bawaan maupun
-- manual lewat EWEHUB:SendWebhook(...). Sengaja dipecah jadi
-- fungsi-fungsi kecil (validasi / builder embed / dispatch) biar
-- gampang dites & dikembangkan terpisah-pisah.
--============================================================
local Webhook = {}

local WEBHOOK_TIMEOUT = 8 -- detik, soft-timeout tiap percobaan kirim

-- Validasi ringan: URL harus format webhook Discord yang valid.
function Webhook.isValidUrl(url)
	if typeof(url) ~= "string" or url == "" then return false end
...
local function Color3ToDecimal(color)
	if typeof(color) ~= "Color3" then return 0x00FF8C end
	return math.floor(color.R * 255) * 65536 + math.floor(color.G * 255) * 256 + math.floor(color.B * 255)
end

-- Susun 1 Discord embed modern: Header/Author, Field, Accent Color,
-- Footer (+icon opsional), Thumbnail/Icon, dan Timestamp otomatis.
function Webhook.buildEmbed(theme, payload)
	local fields = {}
	for _, f in ipairs(payload.Fields or {}) do
		table.insert(fields, { name = f.Name or "-", value = tostring(f.Value or "-"), inline = f.Inline == true })
	end

	local embed = {
		title = payload.Title,
		description = payload.Description,
		color = Color3ToDecimal(payload.Color or theme.Accent),
		fields = fields,
	}

	if payload.AuthorName then
		embed.author = { name = payload.AuthorName, icon_url = payload.AuthorIcon }
	end

	if payload.Footer then
		if typeof(payload.Footer) == "table" then
			embed.footer = { text = payload.Footer.Text, icon_url = payload.Footer.IconUrl }
		else
			embed.footer = { text = payload.Footer }
		end
	end

	local thumbnailUrl = payload.Thumbnail or payload.Icon
	if thumbnailUrl then embed.thumbnail = { url = thumbnailUrl } end

	if payload.Timestamp ~= false then
		local ok, iso = pcall(function() return DateTime.now():ToIsoDate() end)
		if ok then embed.timestamp = iso end
	end

	return embed
end

-- Jalankan 1 percobaan request dengan soft-timeout: dijalankan di
-- coroutine terpisah, thread pemanggil cuma nungguin maksimal
-- WEBHOOK_TIMEOUT detik lalu nyerah (request lama yg akhirnya selesai
-- di background diabaikan dgn aman, gak nge-crash apapun).
local function DispatchWithTimeout(fn, args)
	...
	local elapsed = 0
	while not finished and elapsed < WEBHOOK_TIMEOUT do
		task.wait(0.1)
		elapsed = elapsed + 0.1
	end
...
-- HttpService:PostAsync biasanya DIBLOKIR buat domain di luar Roblox
-- tergantung executor, jadi dicoba beberapa fungsi request umum dulu
-- sebelum fallback ke PostAsync biasa.
function Webhook.dispatch(url, jsonBody)
	local env = (typeof(getgenv) == "function") and getgenv() or _G
	local candidates = {}

	if typeof(env.request) == "function" then table.insert(candidates, env.request) end
	if typeof(env.http_request) == "function" then table.insert(candidates, env.http_request) end
	if typeof(syn) == "table" and typeof(syn.request) == "function" then table.insert(candidates, syn.request) end
	if typeof(fluxus) == "table" and typeof(fluxus.request) == "function" then table.insert(candidates, fluxus.request) end
	...
	for _, fn in ipairs(candidates) do
		local ok, result = DispatchWithTimeout(fn, requestArgs)
		if ok then return true, result end
	end

	local ok2, result2 = DispatchWithTimeout(function()
		return HttpService:PostAsync(url, jsonBody, Enum.HttpContentType.ApplicationJson)
	end, nil)
	if ok2 then return true, result2 end

	return false, result2 or "no request method available"
end

-- EWEHUB:SendWebhook(payload) — payload: {
--   Title, Description, Color (Color3), Fields = {{Name=,Value=,Inline=}},
--   Footer (string ATAU {Text=,IconUrl=}), Thumbnail/Icon (url gambar),
--   AuthorName, AuthorIcon, Username, AvatarUrl,
--   Timestamp (boolean, default true — matiin kalau gak mau timestamp)
-- }
-- Return: ok:boolean, reason:string? — otomatis no-op kalau belum
-- diaktifkan / URL kosong / URL gak valid, aman dipanggil kapan saja
-- tanpa perlu dicek dulu di script pemakai.
function EWEHUB:SendWebhook(payload)
	payload = payload or {}

	if not self.Flags.WebhookEnabled then
		warn("[EWEHUB] SendWebhook dipanggil tapi belum diaktifkan (buka tab 🪝 Webhook).")
		return false, "disabled"
	end

	local url = self.Flags.WebhookUrl
	if not Webhook.isValidUrl(url) then
		warn("[EWEHUB] SendWebhook: URL webhook kosong atau tidak valid.")
		return false, "invalid_url"
	end

	local embed = Webhook.buildEmbed(self.Theme, payload)
	local body = {
		username = payload.Username or (self.Author and (self.Author .. " • EWEHUB") or "EWEHUB"),
	...
	if not okEncode then
		warn("[EWEHUB] Gagal encode payload webhook.")
		return false, "encode_failed"
	end

	local sent, err = Webhook.dispatch(url, encoded)
	if not sent then
		warn("[EWEHUB] Gagal kirim webhook: " .. tostring(err))
	end
...
--============================================================
-- NOTIFICATION SYSTEM (v5.0) — toast yang bisa dikustomisasi penuh:
-- Icon, Accent Color, Progress Bar, posisi, gaya animasi, durasi.
--
-- BACKWARD COMPATIBLE: EWEHUB:Notify({Title=,Content=,Duration=})
-- gaya lama tetap jalan apa adanya — semua opsi baru di bawah ini
-- opsional dengan default yang meniru perilaku versi sebelumnya
-- (Position="BottomRight", Animation="Fade", ShowProgress=false).
--============================================================
local NotifRoots = {} -- keyed by nama posisi, container dibuat lazy per posisi

local NOTIF_POSITION_PRESETS = {
	BottomRight  = { X = 1,   XOffset = -300, YOffset = 10, VAlign = Enum.VerticalAlignment.Bottom },
	TopRight     = { X = 1,   XOffset = -300, YOffset = 10, VAlign = Enum.VerticalAlignment.Top },
	BottomLeft   = { X = 0,   XOffset = 20,   YOffset = 10, VAlign = Enum.VerticalAlignment.Bottom },
	TopLeft      = { X = 0,   XOffset = 20,   YOffset = 10, VAlign = Enum.VerticalAlignment.Top },
	BottomCenter = { X = 0.5, XOffset = -140, YOffset = 10, VAlign = Enum.VerticalAlignment.Bottom },
	TopCenter    = { X = 0.5, XOffset = -140, YOffset = 10, VAlign = Enum.VerticalAlignment.Top },
}

-- Container per posisi dibuat sekali, dipakai ulang. Beberapa toast di
-- posisi yang sama otomatis TERTUMPUK RAPI lewat UIListLayout — itulah
-- "queue system"-nya: gak butuh antrian manual, tumpukan visual SUDAH
-- jadi antrian (toast baru masuk, toast lama tetap ada sampai durasinya habis).
local function EnsureNotifRoot(positionName)
	positionName = (positionName and NOTIF_POSITION_PRESETS[positionName]) and positionName or "BottomRight"
	local preset = NOTIF_POSITION_PRESETS[positionName]

	local existing = NotifRoots[positionName]
	if existing and existing.Gui.Parent then return existing.Container end

	local gui = NewTopScreenGui("EWEHUB_Notifications_" .. positionName)
	local container = New("Frame", {
		Size = UDim2.new(0, 280, 1, -20),
		Position = UDim2.new(preset.X, preset.XOffset, 0, preset.YOffset),
		BackgroundTransparency = 1,
		Parent = gui,
	})
	New("UIListLayout", {
		SortOrder = Enum.SortOrder.LayoutOrder,
		VerticalAlignment = preset.VAlign,
		Padding = UDim.new(0, 8),
		Parent = container,
	})

	NotifRoots[positionName] = { Gui = gui, Container = container }
	return container
end

-- EWEHUB:Notify(config) — config: {
--   Title, Content, Duration (detik, default 4),
--   Icon (emoji/teks pendek, opsional),
--   Color (Color3, accent border+judul+progress, default Theme.Accent),
--   Position ("TopRight"|"TopLeft"|"BottomRight"|"BottomLeft"|
--             "TopCenter"|"BottomCenter", default "BottomRight"),
--   Animation ("Fade"|"Slide"|"Pop", default "Fade"),
--   ShowProgress (boolean, tampilkan progress bar countdown, default false)
-- }
function EWEHUB:Notify(config)
	...
	local animationStyle = config.Animation or "Fade"
	local container = EnsureNotifRoot(config.Position)
	...
	local HeaderRow = New("Frame", { Size = UDim2.new(1, 0, 0, 18), BackgroundTransparency = 1, Parent = Inner })

	local titleOffsetX = 0
	if config.Icon then
		New("TextLabel", {
			Text = config.Icon, Font = Enum.Font.GothamBold, TextSize = 14, TextColor3 = accentColor,
			BackgroundTransparency = 1, TextXAlignment = Enum.TextXAlignment.Left,
			Size = UDim2.new(0, 18, 1, 0), Parent = HeaderRow,
		})
		titleOffsetX = 22
	end
	...
	New("TextLabel", {
		...
		Font = Enum.Font.GothamBold, TextSize = 14, TextColor3 = accentColor,
		BackgroundTransparency = 1, TextXAlignment = Enum.TextXAlignment.Left,
		Position = UDim2.new(0, titleOffsetX, 0, 0),
		Size = UDim2.new(1, -titleOffsetX, 1, 0),
		Parent = HeaderRow,
	})
	...
	if config.ShowProgress then
		local Track = New("Frame", { Size = UDim2.new(1, 0, 0, 3), BackgroundColor3 = theme.Stroke, Parent = Inner }, { Corner(2) })
		ProgressFill = New("Frame", { Size = UDim2.new(1, 0, 1, 0), BackgroundColor3 = accentColor, Parent = Track }, { Corner(2) })
	end

	-- ===== Animasi masuk (smooth transition, 3 gaya) =====
	if animationStyle == "Pop" then
		local scaleObj = New("UIScale", { Scale = 0, Parent = Toast })
		Tween(Toast, FastTween, { BackgroundTransparency = 0 })
		Tween(scaleObj, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 })
	elseif animationStyle == "Slide" then
		Toast.Size = UDim2.new(0, 0, 0, 0) -- Y tetap dihitung AutomaticSize, cuma X yg di-reveal
		Tween(Toast, FastTween, { BackgroundTransparency = 0 })
		Tween(Toast, MediumTween, { Size = UDim2.new(1, 0, 0, 0) })
	else -- "Fade" (default — paling mirip perilaku v4.x, backward-compatible)
	...
	-- ===== Animasi keluar (otomatis setelah `duration`) =====
	task.delay(duration, function()
		if not (Toast and Toast.Parent) then return end
		Tween(Toast, MediumTween, { BackgroundTransparency = 1 })
		if animationStyle == "Slide" then
			Tween(Toast, MediumTween, { Size = UDim2.new(0, 0, 0, 0) })
		end
...
function EWEHUB:CreateWindow(config)
	...
	function Window:CreateTab(tabConfig)
		...
		-- Tab:CreateNote({ Title = "...", Content = "..." })
		-- Komponen INFORMASI MURNI: cuma nampilin teks, gak bisa diedit
		-- user, gak ada Callback, gak dukung Lua/Markdown/HTML/RichText
		-- apapun (RichText Roblox sengaja dibiarkan default/false — teks
		-- yang mengandung tag apapun bakal tampil apa adanya sebagai teks
		-- biasa, bukan diproses). Cocok buat instruksi/catatan ke user.
		function Tab:CreateNote(opt)
			opt = opt or {}
			...
			if opt.Title and opt.Title ~= "" then
				New("TextLabel", {
					Text = opt.Title, Font = Enum.Font.GothamBold, TextSize = 13, TextColor3 = Theme.Accent,
					TextWrapped = true, BackgroundTransparency = 1, TextXAlignment = Enum.TextXAlignment.Left,
	...
	--------------------------------------------------------------
	-- TAB BAWAAN "🪝 Webhook" — SELALU ADA (pinned) di setiap window,
	-- TAPI fiturnya OPSIONAL: developer/user gak wajib isi URL-nya.
	-- URL & status aktif "terdaftar" lewat Flag (WebhookUrl/WebhookEnabled)
	-- persis kayak kontrol lain — otomatis ikut sistem Save/Load Config
	-- yang sudah ada (tab 💾 Konfig), tanpa perlu API tambahan apapun.
	--------------------------------------------------------------
	do
		local WebhookButton = New("TextButton", {
			Text = "🪝  Webhook",
			Font = Enum.Font.GothamMedium,
			TextSize = 13,
			TextColor3 = Theme.SubText,
		...
		New("UIListLayout", { Padding = UDim.new(0, 8), SortOrder = Enum.SortOrder.LayoutOrder, Parent = WebhookPage })

		WebhookButton.MouseEnter:Connect(function()
			if not WebhookPage.Visible then Tween(WebhookButton, FastTween, { BackgroundColor3 = Theme.Stroke }) end
		end)
		WebhookButton.MouseLeave:Connect(function()
			if not WebhookPage.Visible then Tween(WebhookButton, FastTween, { BackgroundColor3 = Theme.PanelLight }) end
		end)

		local WebhookEntry = { Button = WebhookButton, Page = WebhookPage }
		table.insert(AllPages, WebhookEntry)
		WebhookButton.MouseButton1Click:Connect(function() SwitchTo(WebhookEntry) end)

		New("TextLabel", {
			Text = "Kirim event dari script ke Discord lewat webhook (opsional, gak wajib diisi).",
			Font = Enum.Font.Gotham, TextSize = 11, TextColor3 = Theme.SubText,
			TextWrapped = true, BackgroundTransparency = 1, TextXAlignment = Enum.TextXAlignment.Left,
			Size = UDim2.new(1, 0, 0, 28), Parent = WebhookPage,
		})

		-- ===== URL Webhook =====
		local UrlHolder = New("Frame", { Size = UDim2.new(1, 0, 0, 36), BackgroundColor3 = Theme.Panel, Parent = WebhookPage }, { Corner(10), Stroke() })
		local UrlInput = New("TextBox", {
			Text = "", PlaceholderText = "https://discord.com/api/webhooks/...",
			Font = Enum.Font.Gotham, TextSize = 12, TextColor3 = Theme.Text, PlaceholderColor3 = Theme.SubText,
			BackgroundTransparency = 1, Position = UDim2.new(0, 12, 0, 0), Size = UDim2.new(1, -24, 1, 0),
			TextXAlignment = Enum.TextXAlignment.Left, ClearTextOnFocus = false, Parent = UrlHolder,
		})
		UrlInput.Focused:Connect(function() Tween(UrlHolder, FastTween, { BackgroundColor3 = Theme.PanelLight }) end)
		UrlInput.FocusLost:Connect(function()
			Tween(UrlHolder, FastTween, { BackgroundColor3 = Theme.Panel })
			EWEHUB.Flags.WebhookUrl = UrlInput.Text
		end)

		EWEHUB.Flags.WebhookUrl = EWEHUB.Flags.WebhookUrl or ""
		EWEHUB.ConfigCallbacks.WebhookUrl = function(v)
			UrlInput.Text = v or ""
			EWEHUB.Flags.WebhookUrl = v or ""
		end

		-- ===== Toggle Aktif/Nonaktif =====
		local EnableHolder = New("Frame", { Size = UDim2.new(1, 0, 0, 36), BackgroundColor3 = Theme.Panel, Parent = WebhookPage }, { Corner(10), Stroke() })
		New("TextLabel", {
			Text = "Aktifkan Pengiriman Webhook", Font = Enum.Font.GothamMedium, TextSize = 13, TextColor3 = Theme.Text,
			TextXAlignment = Enum.TextXAlignment.Left, BackgroundTransparency = 1,
			Position = UDim2.new(0, 12, 0, 0), Size = UDim2.new(1, -60, 1, 0), Parent = EnableHolder,
		})

		local webhookEnabled = false
		local Switch = New("Frame", {
			Size = UDim2.new(0, 40, 0, 20), Position = UDim2.new(1, -50, 0.5, -10),
			BackgroundColor3 = Theme.Stroke, Parent = EnableHolder,
		}, { Corner(10) })
		local Knob = New("Frame", {
			Size = UDim2.new(0, 16, 0, 16), Position = UDim2.new(0, 2, 0.5, -8),
			BackgroundColor3 = Color3.fromRGB(255, 255, 255), Parent = Switch,
		}, { Corner(8) })
		local EnableClickArea = New("TextButton", { Text = "", BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), Parent = EnableHolder })

		local function SetWebhookEnabled(state)
			webhookEnabled = state
			Tween(Switch, FastTween, { BackgroundColor3 = state and Theme.Accent or Theme.Stroke })
			Tween(Knob, FastTween, { Position = state and UDim2.new(1, -18, 0.5, -8) or UDim2.new(0, 2, 0.5, -8) })
			EWEHUB.Flags.WebhookEnabled = state
		end

		EnableClickArea.MouseButton1Click:Connect(function() SetWebhookEnabled(not webhookEnabled) end)
		EWEHUB.Flags.WebhookEnabled = false
		EWEHUB.ConfigCallbacks.WebhookEnabled = function(v) SetWebhookEnabled(v == true) end

		-- ===== Test Webhook =====
		local TestBtn = New("TextButton", {
			Text = "🧪 Test Webhook", Font = Enum.Font.GothamBold, TextSize = 12, TextColor3 = Theme.Text,
			BackgroundColor3 = Theme.Panel, Size = UDim2.new(1, 0, 0, 34), AutoButtonColor = false, Parent = WebhookPage,
		}, { Corner(8), Stroke() })
		TestBtn.MouseEnter:Connect(function() Tween(TestBtn, FastTween, { BackgroundColor3 = Theme.AccentDark }) end)
		TestBtn.MouseLeave:Connect(function() Tween(TestBtn, FastTween, { BackgroundColor3 = Theme.Panel }) end)
		TestBtn.MouseButton1Click:Connect(function()
			if not webhookEnabled then
				EWEHUB:Notify({ Title = "Webhook", Content = "Nyalain dulu toggle 'Aktifkan Pengiriman Webhook'.", Duration = 4 })
				return
			end
			if not Webhook.isValidUrl(UrlInput.Text) then
				EWEHUB:Notify({ Title = "Webhook", Content = "URL belum diisi / formatnya bukan webhook Discord yang valid.", Duration = 4 })
				return
			end
			...
			task.spawn(function()
				local sent, err = EWEHUB:SendWebhook({
					Title = "✅ Test Webhook",
					Description = "Kalau kamu lihat pesan ini di Discord, webhook kamu udah kesambung!",
					Color = Theme.Accent,
					AuthorName = EWEHUB.Author and ("EWEHUB v" .. EWEHUB.Version) or nil,
					Footer = "EWEHUB v" .. EWEHUB.Version,
				})
				TestBtn.Text = "🧪 Test Webhook"
				EWEHUB:Notify({
					Title = "Webhook",
					Content = sent and "Test terkirim! Cek channel Discord kamu."
						or ("Gagal kirim (" .. tostring(err) .. "). Cek URL & koneksi."),
					Duration = 4,
				})
			end)
		end)
